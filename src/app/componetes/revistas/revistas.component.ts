...
export class RevistasComponent implements OnInit {
  ...
  AgregarLibro(){
    this.ModeloLibros.tipo = "Revistas"
  ...
  Eliminar(){
    ...
    Swal.fire({
      title: 'Deseas eliminar esta revista?',
      text: "No podras recuperar los datos de esta revista si lo eliminas",
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        this._LibrosServices.DeleteUnLibro(id).subscribe(
          res => {
            ...
            Swal.fire(
              'Eliminado!',
              'Se a Eliminado esta revista de forma correcta',
              'success'
            )
          }, error => {
